...
```{r, fig.align='center', fig.width=3.5, fig.height=3.5, fig.pos='H', fig.cap="Location of the 20 randomly selected ACs (gold) with respect to the trajectory simulated from a BRW (grey). The initial location is denoted by the green circle and the ending location by the red triangle."}
...
These observations from the simulated trajectory will be used to create a grid on which to discretize space before running the segmentation algorithm. Cell resolution will be 2 units.
...
```{r, fig.align='center', fig.width=6, fig.height=6, fig.pos='H', fig.cap="Density surface of point distribution."}
...
```{r, fig.align='center', out.width='0.85\\linewidth', fig.pos='H', fig.cap="Heatmap of grid cell occupancy over 7500 observations. Vertical green lines denote modeled breakpoints."}

include_graphics(path.expand("~/Documents/Snail Kite Project/Data/R Scripts/method-comparison/BRW segmentation heatmap.png"))
```


There are many breakpoints identified by the segmentation model, approximately an order of magnitude greater than the true number of breakpoints. When compared against one another, the breakpoints from the model do overlap with true breakpoints as well as many other additional points.

```{r, fig.align='center', out.width='0.85\\linewidth', fig.pos='H', fig.cap="Comparison of true breakpoints from the BRW simulation compared to those identified by the model. N = 28 for true breakpoints and N = 278 for modeled breakpoints."}

include_graphics(path.expand("~/Documents/Snail Kite Project/Data/R Scripts/method-comparison/BRW breakpoint comparison.png"))
```

Although there are many more breakpoints identified by the model than expected, we will continue with this example. Next, time segments will be assigned to the data, which will serve as input for the mixture model used to identify and assign ACs.
...
From these results, it appears that the model had difficulty identifying the correct location of the ACs in just about every instance. This may be due to the way the trajectory was simulated and its effect on the segmentation step.
...
```{r, fig.align='center', out.width='0.75\\linewidth', fig.pos='H', fig.cap="The model identifies fewer breakpoints than the true breakpoints. The locations of the modeled breakpoints are very similar if not identical to the true positions. However, some modeled breakpoints appear to be redundant."}
...
c(0.546, 0.266, 0.187, 0.002, 0.000, 0.000, 0.000)
```
...
```{r, fig.align='center', out.width='1.0\\linewidth', fig.pos='H', fig.cap="Simulated hard-clustering CRW track with behaviors estimated by the model. The green circle indicates the starting location and the red triangle is the ending location."}
...
At a quick glance, the segmentation appears to do a decent job compared to the true breakpoints, but still misses a number of breaks where there are transitions between 'resting' and 'exploratory' behaviors. Next, I will run the latent Dirichlet allocation (LDA) model to cluster the time segments into separate behaviors.
...
#Determine proportion of behaviors (across all time segments)
#Possibly set threshold below which behaviors are excluded
c(0.563, 0.281, 0.153, 0.003, 0.000, 0.000, 0.000)
```
...
The modeled proportions appear to be different compared to the true values used to generate the simulation. This could be a result of the segmentation step and how different behaviors were grouped together. Next, let's look at the histograms for each of these top three clusters/behaviors from the model and assign a state to each:
...
These histograms are not as clean as for the hard-clustering CRW simulation, but still match up with the original distributions of the mixed-membership simulation. Now, we can investigate how these behavioral estimates match with the true behaviors from the mixed-membership simulation:
...
Results for all three behaviors show that the model does an okay job at estimating the proportion of each behavior across all time segments, likely due to the difficulty distinguishing between 'resting' and 'exploratory' behaviors. This is likely a result of the 'resting' and 'exploratory' clusters having SL and TA distributions that were very similar to each other following the segmentation of the time series. Alternatively, it appears that the 'transit' behavior was assigned properly most often of the time; estimates were often in greater proportions compared to the simulated data, however.
...
Outwardly, the behavioral states on this track as estimated by the model still very similar to the original simulation, but there are sections were differences are noticeable. To quantify the model's accuracy, we will directly compare the behavioral estimates for all time segments of the track.
\hfill\break

The overall accuracy of the model in identifying the dominant behavior per time segment was 82.0% (including all behaviors together). When breaking this down by behavior, the 'exploratory' behavior was identified with the least accuracy (64.8% accurate), which was much lower than for the hard-clustering simulation (83%). The 'resting' and 'transit' behaviors were both more accurate (88.6 and 98.5%, respectively) than the 'exploratory' behavior, but were noticeably lower than the 99% classification accuracy of both behaviors from the hard-clustering simulation. It appears that the 'resting' and 'transit' behaviors were more easily identified since they were much different from each other and represented extremes. As an intermediate behavior type that resembled the 'resting' behavior in SL and TA after the segmentation step of the model, the 'exploratory' behavior was not accurately classified nearly as much as in the previous simulation. This could also be a possible effect of the simulation itself during the step of simulating the different states per observation within each time segment if there were similar values sampled for SL and TA in the 'resting' and 'exploratory' behaviors. Overall, this model does a pretty good job at classification of the extreme behaviors, but struggled to classify the intermediate behavior for the mixed-membership CRW simulation.